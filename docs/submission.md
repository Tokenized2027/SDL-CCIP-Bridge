...
2. **Bridge AI Advisor** reads vault state via EVMClient, then calls an AI analysis endpoint (Claude Haiku) via HTTPClient with `consensusIdenticalAggregation`. All DON nodes must agree on the AI response before it's accepted. The AI recommends policy parameter adjustments (utilization cap, reserve cut, hot reserve target).
...
- **Claude Haiku**: structured risk assessment via Flask endpoint
