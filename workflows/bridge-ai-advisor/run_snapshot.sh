...
SNAPSHOT_PATH="${SNAPSHOT_PATH:-${ROOT_DIR}/../../intelligence/data/cre_bridge_advisor_snapshot.json}"
...
def parse_output(text: str) -> dict | None:
    marker_matches = re.findall(r"BRIDGE_ADVISOR_CRE_OUTPUT_JSON=(\{.*\})", text)
    if marker_matches:
        try:
    ...
    for idx in range(len(text)):
        try:
            ...
            continue
        if isinstance(obj, dict) and "vaultState" in obj:
            return obj
    return None
...
if not parsed:
    raise SystemExit("Could not find structured bridge-ai-advisor workflow output")
...
print(f"Wrote bridge-ai-advisor snapshot to {snapshot_path}")
PY
