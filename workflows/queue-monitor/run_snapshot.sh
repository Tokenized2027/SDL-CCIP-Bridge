...
SNAPSHOT_PATH="${SNAPSHOT_PATH:-${ROOT_DIR}/../../intelligence/data/cre_queue_monitor_snapshot.json}"
...
def parse_output(text: str) -> dict | None:
    marker_matches = re.findall(r"QUEUE_MONITOR_CRE_OUTPUT_JSON=(\{.*\})", text)
    if marker_matches:
        try:
    ...
    for idx in range(len(text)):
        try:
            ...
            continue
        if isinstance(obj, dict) and "queue" in obj:
            return obj
    return None
...
if not parsed:
    raise SystemExit("Could not find structured queue-monitor workflow output")
...
print(f"Wrote queue-monitor snapshot to {snapshot_path}")
PY
